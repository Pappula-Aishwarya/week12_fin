...
def test_empty_password(setup_teardown):
    driver = setup_teardown
    driver.get("http://127.0.0.1:5001/")
    driver.find_element(By.NAME, "username").send_keys("Aishwarya")
...
def test_short_password(setup_teardown):
    ...
    driver.get("http://127.0.0.1:5001/")
    driver.find_element(By.NAME, "username").send_keys("Aishwarya")
    driver.find_element(By.NAME, "pwd").send_keys("aish")
    driver.find_element(By.NAME, "sb").click()
    time.sleep(1)
    alert_text = get_alert_text(driver)
...
def test_valid_input(setup_teardown):
    ...
    driver.get("http://127.0.0.1:5001/")
    driver.find_element(By.NAME, "username").send_keys("Aishwarya")
    driver.find_element(By.NAME, "pwd").send_keys("aish123")
    driver.find_element(By.NAME, "sb").click()
    time.sleep(2)
    current_url = driver.current_url
    assert "/submit" in current_url
    body_text = driver.find_element(By.TAG_NAME, "body").text
    assert "Hello, Aishwarya! Welcome to the website" in body_text
